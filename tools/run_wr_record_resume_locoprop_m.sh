...
case "${generator_kind}" in
  aux)
    generator_script="tools/make_wr_record_resume_locoprop_m_aux.py"
    default_generated="/tmp/train_gpt_wr_record_resume_locoprop_m_aux_${steps}.py"
    ;;
  hook)
    echo "hook resume LocoProp generator is not part of the committed runner; use WR_LOCOM_GENERATOR=aux" >&2
    exit 2
    ;;
  *)
...
generated_script="${WR_RESUME_LOCOM_GENERATED_SCRIPT:-${default_generated}}"

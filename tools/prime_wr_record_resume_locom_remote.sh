...
label="${RUN_LABEL:-wr_record_resume_locom_probe}"
...
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-/root/.cache/torchinductor-wr-record-resume-locom-${nproc}x}"
...
generated="/root/prime_track3_logs/${label}_train_gpt_wr_record_resume_locom.py"
WR_RESUME_LOCOM_DRY_RUN=1 \
WR_RESUME_LOCOM_GENERATED_SCRIPT="${generated}" \
...
bash tools/run_wr_record_resume_locoprop_m.sh > "/root/prime_track3_logs/${label}_generate.log" 2>&1
...
status_file="/root/prime_track3_logs/${label}.status"
{
  ...
  printf 'optimizer2_adjust momentum_scale=%s soap_reset=%s nor_reset=%s\n' \
    "${WR_RESUME_OPTIMIZER2_MOMENTUM_SCALE:-1.0}" \
    "${WR_RESUME_OPTIMIZER2_SOAP_RESET:-0}" \
    "${WR_RESUME_OPTIMIZER2_NOR_RESET:-0}"
  printf 'save checkpoint=%s save_step=%s save_steps=%s\n' \
    "${WR_SAVE_CHECKPOINT:-}" "${WR_SAVE_CHECKPOINT_STEP:-}" "${WR_SAVE_CHECKPOINT_STEPS:-}"
  printf 'wr_locom enabled=%s layers=%s windows=%s K=%s interval=%s sample_tokens=%s aux_seqs=%s batched_prep=%s inner_lr=%s prox=%s alpha=%s norm_cap=%s norm_to_base=%s min_cos=%s require_loss_decrease=%s\n' \
    "${WR_LOCOM_ENABLED:-1}" "${WR_LOCOM_LAYERS:-all}" "${WR_LOCOM_ACTIVE_WINDOWS:-}" \
    "${WR_LOCOM_STEPS:-4}" "${WR_LOCOM_INTERVAL:-1}" "${WR_LOCOM_SAMPLE_TOKENS:-1024}" "${aux_seqs}" "${batched_prep}" "${WR_LOCOM_INNER_LR:-0.1}" \
...
env \
  ...
  WR_RESUME_OPTIMIZER2_MOMENTUM_SCALE="${WR_RESUME_OPTIMIZER2_MOMENTUM_SCALE:-1.0}" \
  WR_RESUME_OPTIMIZER2_SOAP_RESET="${WR_RESUME_OPTIMIZER2_SOAP_RESET:-0}" \
  WR_RESUME_OPTIMIZER2_NOR_RESET="${WR_RESUME_OPTIMIZER2_NOR_RESET:-0}" \
  ...
  WR_SAVE_CHECKPOINT_STEPS="${WR_SAVE_CHECKPOINT_STEPS:-}" \
  ...
  bash tools/run_wr_record_resume_locoprop_m.sh > "${run_log}" 2>&1

...
# Prime H100 probe for the "is the late failure missing WR optimizer
# machinery?" hypothesis.
#
# Flow:
#   1. Copy the known step-1600 LocoProp checkpoint.
#   2. Recreate the good K5/coldp2 prefix to step 1800 and save it.
#   3. From the exact same step-1800 state, run:
#      a. simple coldp2 continuation with the original optimizer state
#      b. current-record WR stack continuation with model+Adam loaded, fresh
#         WR optimizer2 so it allocates the full 324-tensor state shape.
#   4. Save val curves and lightweight optimizer-state diagnostics through 2500.
#
# The script launches the remote workload under nohup and exits after startup.
# It intentionally does not terminate the pod; terminate only after pulling the
...
pod_name="${PRIME_POD_NAME:-oc-main-track3-wrstack1800-h100-$(date -u +%Y%m%d-%H%M)}"
remote_repo="${REMOTE_REPO:-/root/wr-track3-locom-20260606}"
remote_logdir="${REMOTE_LOGDIR:-/root/prime_track3_wrstack_from1800_logs}"
remote_ckpt_dir="${REMOTE_CKPT_DIR:-/root/.cache/track3_checkpoints}"
known_hosts="${PRIME_KNOWN_HOSTS:-${workspace}/.opencode/prime_known_hosts_wrstack1800}"
disk_size_gb="${PRIME_DISK_SIZE_GB:-500}"
min_balance="${PRIME_MIN_BALANCE_USD:-1.00}"
...
- pod_id: ${pod_id}
  name: ${pod_name}
  owner: current-agent
  purpose: Track 3 same-step1800 simple-vs-WR-stack continuation to 2500; regenerate K5/coldp2 1800 first
  gpu: ${availability_desc}
  price_per_hour: selected by availability id ${availability_id}
  created_at: ${created_at}
  expected_stop: after logs and state diagnostics through 2500 are pulled
...
status_dir="${workspace}/.opencode/current_track3_ledger_20260608_logs/wrstack_from1800_${pod_id}"
mkdir -p "${status_dir}"
...
echo "writing remote wrstack-from-1800 run"
ssh_remote "${ssh_target}" "cat > '${remote_logdir}/run_wrstack_from1800.sh'" <<REMOTE_SCRIPT
#!/usr/bin/env bash
set -euo pipefail
...
seed=3710
source1600=${remote_ckpt}
ckpt1800="\${ckpt_dir}/track3_k5_coldp2_seed3710_to1800_seed3710_step1800.pt"
...
export TORCHINDUCTOR_CACHE_DIR=/root/.cache/torchinductor-track3-wrstack1800
...
echo "stage=regen1800 start=\$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "\${logdir}/RUNNING"
TRACK3_KDIAG_PROFILE=post-true-k10-lr2e4-active-poscos \\
TRACK3_KDIAG_LABEL=track3_k5_coldp2_seed3710_to1800 \\
TRACK3_KDIAG_LOG_DIR="\${logdir}" \\
TRACK3_SOURCE=records/track_3_optimization/train_gpt_simple.py \\
TRACK3_GENERATED_SCRIPT=/tmp/track3_k5_coldp2_seed3710_to1800.py \\
TRACK3_TRAIN_STEPS=1800 \\
TRACK3_TARGET_LOSS=0 \\
TRACK3_NUM_TRIALS=1 \\
TRACK3_MBS=16 \\
TRACK3_SEED_BASE=0 \\
TRACK3_SEED_OFFSET=3710 \\
TRACK3_RESUME_CHECKPOINT="\${source1600}" \\
...
TRACK3_LOCOM_LOG_STEPS=1600,1625,1700,1750,1800 \\
TRACK3_LOCOM_DIAG_STEPS=1,2,3,4,5 \\
TRACK3_LOCOM_PREP_MAX_STATS=12 \\
TRACK3_LOCOM_APPLY_MAX_STATS=12 \\
TRACK3_CHECKPOINT_STEPS=1800 \\
TRACK3_CHECKPOINT_DIR="\${ckpt_dir}" \\
TRACK3_CHECKPOINT_PREFIX=track3_k5_coldp2_seed3710_to1800 \\
...
SCREEN_VAL_EVERY=25 \\
NPROC_PER_NODE=1 \\
bash tools/run_track3_locom_kdiag_probe.sh > "\${logdir}/regen1800.outer.log" 2>&1

if [[ ! -f "\${ckpt1800}" ]]; then
  echo "missing regenerated checkpoint \${ckpt1800}" | tee "\${logdir}/FAILED"
  exit 10
fi

echo "stage=simplecold start=\$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "\${logdir}/RUNNING"
TRACK3_SOURCE=records/track_3_optimization/train_gpt_simple.py \\
TRACK3_GENERATED_SCRIPT=/tmp/track3_simplecold_from1800_to2500.py \\
TRACK3_TRAIN_STEPS=2500 \\
TRACK3_TARGET_LOSS=0 \\
TRACK3_NUM_TRIALS=1 \\
TRACK3_MBS=16 \\
TRACK3_SEED_BASE=0 \\
TRACK3_SEED_OFFSET=3710 \\
TRACK3_RESUME_CHECKPOINT="\${ckpt1800}" \\
TRACK3_RESUME_ADVANCE_DATA=1 \\
TRACK3_RESUME_RESTORE_RNG=1 \\
TRACK3_RESUME_LOAD_OPTIMIZERS=1 \\
...
TRACK3_CHECKPOINT_STEPS=2000,2125,2250,2400,2500 \\
TRACK3_CHECKPOINT_DIR="\${ckpt_dir}" \\
TRACK3_CHECKPOINT_PREFIX=track3_simplecold_from1800_seed3710 \\
TRACK3_CHECKPOINT_EXIT_AFTER=0 \\
TRACK3_COOLDOWN_FRAC=1.0 \\
TRACK3_LR_SCHEDULE=power \\
TRACK3_LR_POWER=2.0 \\
...
SCREEN_VAL_EVERY=25 \\
NPROC_PER_NODE=1 \\
bash tools/run_track3_locoprop_m.sh > "\${logdir}/simplecold_from1800_to2500.log" 2>&1

echo "stage=wrstack start=\$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "\${logdir}/RUNNING"
WR_TRAIN_STEPS=2500 \\
...
WR_SAVE_CHECKPOINT="\${ckpt_dir}/wrstack_from1800_seed3710_step{step}.pt" \\
WR_SAVE_CHECKPOINT_STEPS=2000,2125,2250,2400,2500 \\
WR_TARGET_LOSS=0 \\
SCREEN_VAL_EVERY=25 \\
NPROC_PER_NODE=1 \\
bash tools/run_wr_record_resume.sh > "\${logdir}/wrstack_from1800_to2500.log" 2>&1
...
paths = []
for pat in [
    "/root/.cache/track3_checkpoints/track3_k5_coldp2_seed3710_to1800_seed3710_step1800.pt",
    "/root/.cache/track3_checkpoints/track3_simplecold_from1800_seed3710_seed3710_step*.pt",
    "/root/.cache/track3_checkpoints/wrstack_from1800_seed3710_step*.pt",
]:
    paths.extend(glob.glob(pat))

def sort_key(path):
    m = re.search(r"step(\\d+)", os.path.basename(path))
    return (0 if "k5_coldp2" in path else 1 if "simplecold" in path else 2, int(m.group(1)) if m else -1)
...
rows = []
for label, path in [
    ("regen1800", "\${logdir}/regen1800.outer.log"),
    ("simplecold", "\${logdir}/simplecold_from1800_to2500.log"),
    ("wrstack", "\${logdir}/wrstack_from1800_to2500.log"),
]:
    ...
    for line in open(path, errors="replace"):
        ...
        if m:
            rows.append((label, int(m.group(1)), int(m.group(2)), float(m.group(3)), float(m.group(4))))
print("label\\tstep\\ttotal\\tval_loss\\tstep_avg_ms")
for row in rows:
    print("{}\\t{}\\t{}\\t{:.5f}\\t{:.2f}".format(*row))
PY
...
ssh_remote "${ssh_target}" "chmod +x '${remote_logdir}/run_wrstack_from1800.sh' && nohup '${remote_logdir}/run_wrstack_from1800.sh' > '${remote_logdir}/run.nohup.log' 2>&1 & echo \$! > '${remote_logdir}/run.pid'"
run_started=1
...
stages=regen1800,simplecold_to2500,wrstack_to2500
EOF

...
# Launch a single late-suffix Track 3 screen from the saved step-2400 state.
# This intentionally avoids replaying the 1600->2000 bracket.

workspace="${WORKSPACE:-$(pwd)}"
env_file="${PRIME_ENV_FILE:-/Users/progyan/speedrun/env.local}"
ledger="${PRIME_LEDGER:-${workspace}/.opencode/prime-gpu-ledger.md}"
pod_name="${PRIME_POD_NAME:-oc-main-track3-late-linear-h100-$(date -u +%Y%m%d-%H%M)}"
remote_repo="${REMOTE_REPO:-/root/wr-track3-locom-20260606}"
remote_logdir="${REMOTE_LOGDIR:-/root/prime_track3_late_linear_probe_logs}"
remote_ckpt_dir="${REMOTE_CKPT_DIR:-/root/.cache/track3_checkpoints}"
known_hosts="${PRIME_KNOWN_HOSTS:-${workspace}/.opencode/prime_known_hosts_late_linear}"
disk_size_gb="${PRIME_DISK_SIZE_GB:-500}"
min_balance="${PRIME_MIN_BALANCE_USD:-3.00}"
gpu_type="${PRIME_GPU_TYPE:-H100_80GB}"
gpu_socket_filter="${PRIME_GPU_SOCKET:-SXM}"
local_ckpt="${LOCAL_CKPT:-/Users/progyan/speedrun/tmp/prime_recover2000_checkpoints/track3_short2000_wr3105p120_mult035_hold2400_seed3710_step2400.pt}"
remote_ckpt="${remote_ckpt_dir}/track3_short2000_wr3105p120_mult035_hold2400_seed3710_step2400.pt"
suffix_lanes="${TRACK3_SUFFIX_LANES:-linear3000_noloco}"
...
- pod_id: ${pod_id}
  name: ${pod_name}
  owner: current-agent
  purpose: Track 3 late linear 3000-horizon suffix screen from step2400 checkpoint; lanes=${suffix_lanes}
...
echo "writing remote late-linear run"
ssh_remote "${ssh_target}" "cat > '${remote_logdir}/run_late_linear.sh'" <<REMOTE_SCRIPT
#!/usr/bin/env bash
set -euo pipefail
...
ssh_remote "${ssh_target}" "chmod +x '${remote_logdir}/run_late_linear.sh' && nohup '${remote_logdir}/run_late_linear.sh' > '${remote_logdir}/run.nohup.log' 2>&1 & echo \$! > '${remote_logdir}/run.pid'"
run_started=1

...
profiles=(post-approx-k10-alpha0 pre-k10-alpha0 post-true-k10-alpha0)
...
for profile in "${profiles[@]}"; do
  label=track3_kdiag_${profile}
  echo "profile_start ${profile} $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$logdir/sequence.status"
  TRACK3_KDIAG_PROFILE="${profile}" TRACK3_KDIAG_LABEL="${label}" TRACK3_KDIAG_LOG_DIR="$logdir" bash tools/run_track3_locom_kdiag_probe.sh
  echo "profile_done ${profile} $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$logdir/sequence.status"
done
touch "$logdir/DONE"

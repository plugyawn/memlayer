...
label=track3_from2400_wr3105p120_hold2450_fade2750_seed3710
...
export TORCHINDUCTOR_CACHE_DIR=/home/ubuntu/.cache/torchinductor-track3-from2400-wr3105p120-hold2450-fade2750

{
  echo "started $(date -Is)"
  echo "resume=$ckpt_dir/track3_short2000_wr3105p120_mult035_hold2400_seed3710_step2400.pt"
  echo "tail=pr287 h3105 p1.20; resume at 2400; LR multiplier 0.35 held 2400-2450 then fade to 1 by 2750; no late LocoProp"
  echo "gate: kill if 2500 > 3.3430, or 2600 is not clearly better than the old slow-fade 3.33852"
} > "$logdir/${label}.status"
...
python3 -c 'import os, sys; os.setsid(); os.execvp(sys.argv[1], sys.argv[1:])' env \
  ...
  TRACK3_CHECKPOINT_PREFIX=track3_from2400_wr3105p120_hold2450_fade2750 \
  ...
  TRACK3_LOCOM_LOG_STEPS=2400,2425,2450,2475,2500,2525,2550,2575,2600,2625,2650,2700,2750,2800,2875,2950,3000 \
  SCREEN_VAL_EVERY=25 \
  NPROC_PER_NODE=1 \
...
python3 tools/watch_track3_gates.py \
  ...
  --max-loss 2500:3.3430 \
  --max-loss 2600:3.33852 \
  --max-loss 2750:3.3150 \
  --max-loss 2875:3.2880 &
watch_pid=$!

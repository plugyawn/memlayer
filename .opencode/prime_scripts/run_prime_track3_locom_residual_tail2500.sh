...
# Targeted mechanism probe: solve the LocoProp-M local objective after the base
# Muon update has already been applied, so the correction is a residual on top
# of Muon rather than an independent raw local-target displacement.
...
log_dir="${TRACK3_RESIDUAL_LOG_DIR:-/root/prime_track3_locom_residual_tail2500_logs_43ad37}"
...
label="track3_locom_tail2500_residual_norm005_seed${seed_offset}_to${train_steps}"
...
python3 - <<'PY' "${logfile}" | tee "${log_dir}/summary.tsv"
import re
import sys
...
echo "track3_locom_residual_tail2500_done $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
